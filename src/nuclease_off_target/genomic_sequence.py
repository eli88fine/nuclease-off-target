...
from .exceptions import IsoformInDifferentChromosomeError,UrlNotImplementedForGenomeError
...
def _extract_genome_build_from_ucsc_response_header_line(sequence_info_line:str)->str:
    match = GENOME_BUILD_IN_RESPONSE_HEADER_REGEX.search(sequence_info_line)
    if match is None:
        raise NotImplementedError(f'The line did not contain a genome: {sequence_info_line}')
    return match[1]
...
def request_sequence_from_ucsc(url: str, expected_genome:str) -> str:
    ...
    actual_genome = _extract_genome_build_from_ucsc_response_header_line(sequence_info_line)
    if actual_genome != expected_genome:
        raise DnaRequestGenomeMismatchError(actual_genome,expected_genome)
    print (sequence_info_line)
